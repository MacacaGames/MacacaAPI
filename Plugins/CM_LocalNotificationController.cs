// Port form https://github.com/Agasper/unity-android-notifications
using System;
using System.IO;
using UnityEngine;
#if UNITY_IOS
...
{
    [System.Obsolete("CM_LocalNotificationController is Obsolete use Unity's MobileNotification instead")]
    public class CM_LocalNotificationController
    {

#if UNITY_ANDROID && !UNITY_EDITOR
	private static string fullClassName = "com.CloudMacaca.AndroidNative.UnityNotificationManager";
	private static string actionClassName = "com.CloudMacaca.AndroidNative.NotificationAction";
	private static AndroidJavaClass UnityNotificationManager = new AndroidJavaClass(fullClassName);
#endif
	...
	private static string bundleIdentifier { get { return Application.bundleIdentifier; } }
#endif
        public static string smallIcon = "app_icon";
        ...
        public static int SendNotification(int id, long delayMs, string title, string message, Color32 bgColor, bool sound = true, bool vibrate = true, bool lights = true, string bigIcon = "", String soundName = null, string channel = "default", params Action[] actions)
        {
			...
			{
				UnityNotificationManager.CallStatic("SetNotification", id, delayMs, title, message, message,
        ...
        public static int SendRepeatingNotification(int id, long delayMs, long timeoutMs, string title, string message, Color32 bgColor, bool sound = true, bool vibrate = true, bool lights = true, string bigIcon = "", String soundName = null, string channel = "default", params Action[] actions)
        {
			...
			if (UnityNotificationManager != null)
			{
				UnityNotificationManager.CallStatic("SetRepeatingNotification", id, delayMs, title, message, message, timeoutMs,
				sound ? 1 : 0, soundName, vibrate ? 1 : 0, lights ? 1 : 0, bigIcon, smallIcon,
				ToInt(bgColor), bundleIdentifier, channel, PopulateActions(actions));
			}
			return id;
...
#if UNITY_ANDROID && !UNITY_EDITOR
			if (UnityNotificationManager != null) {
				UnityNotificationManager.CallStatic("CancelPendingNotification", id);
			}
#elif UNITY_IOS && !UNITY_EDITOR
...
#if UNITY_ANDROID && !UNITY_EDITOR
			if (UnityNotificationManager != null) {
				UnityNotificationManager.CallStatic("ClearShowingNotifications");
			}
#elif UNITY_IOS && !UNITY_EDITOR
		...
		if (UnityNotificationManager != null)
		{
			UnityNotificationManager.CallStatic("CreateChannel", identifier, name, description, (int) importance, soundName, enableLights ? 1 : 0, ToInt(lightColor), vibrate ? 1 : 0, vibrationPattern, bundleIdentifier);
		}
#endif
        }
