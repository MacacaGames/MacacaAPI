...
#endif
{
    ...
    public class ProjectCapabilityManager
    {
        ...
        public void AddSignInWithApple(bool AddSignInRequire = false)
        {
            var entitlementDoc = GetOrCreateEntitlementDoc();
            ...
            {
                var capabilityType =
                project.AddCapability(m_TargetGuid, PBXCapabilityType.AppleSignIn, m_EntitlementFilePath, !AddSignInRequire);
                //project.AddFrameworkToProject(m_TargetGuid, AuthenticationServicesFramework, !AddSignInRequire);
            }
        }
        ...
        private PlistDocument GetOrCreateEntitlementDoc()
        {
            if (m_Entitlements == null)
            {
                ...
                string[] entitlementsFiles;
                try
                {
                    entitlementsFiles = Directory.GetFiles(m_BuildPath, m_EntitlementFilePath);
                ...
                {
                    UnityEngine.Debug.LogError("Try to open current file error create new one");
