...
public static class InfoPlistProcessor
{
    ...
    private static void SetPrivacySensiticeData(PlistDocument plist, List<XcodeProjectSetting.PrivacyData> permission)
    {
        PlistElementDict rootDict = plist.root;
        int count = permission.Count;
        for (int i = 0; i < count; i++)
        {
            rootDict.SetString(permission[i].name, permission[i].description);
        }
    }
    ...
    public static void SetSKAdNetwork(PlistDocument plist, List<XcodeProjectSetting.SKAdNetworkItems> datas)
    {
        PlistElementDict rootDict = plist.root;
        PlistElementArray skAd = rootDict.CreateArray("SKAdNetworkItems");
        foreach (var item in datas)
        {
            var dict = skAd.AddDict();
            dict.SetString("SKAdNetworkIdentifier", item.value);
        }
    }
